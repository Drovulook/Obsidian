...
App::App() {
  m_globalDescriptorPool =
      ODDescriptorPool::Builder(m_device)
          .setMaxSets(ODSwapChain::MAX_FRAMES_IN_FLIGHT)
          .addPoolSize(
              VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
              ODSwapChain::MAX_FRAMES_IN_FLIGHT) // contient les infos de la
                                                 // caméra et de l'éclairage
          .addPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
                       ODSwapChain::MAX_FRAMES_IN_FLIGHT) // texture
          .addPoolSize(VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
                       ODSwapChain::MAX_FRAMES_IN_FLIGHT *
                           2) // buffer compute.comp <-> compute.vert et .frag
          .addPoolSize(
              VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER,
              ODSwapChain::MAX_FRAMES_IN_FLIGHT) // info de pas de temps pour le
                                                 // compute shader
          .build();
...
App::~App() {
  ...
  if (!m_transitionCommandBuffers.empty()) {
    vkFreeCommandBuffers(
        m_device.device(), m_device.getCommandPool(),
...
void App::run() {
  ...
  m_uiManager->init(
      m_window.getGLFWWindow(), m_device.device(), m_device.physicalDevice(),
  ...
  std::vector<std::unique_ptr<ODBuffer>> uboComputeBuffers(
      ODSwapChain::MAX_FRAMES_IN_FLIGHT);
  for (int i = 0; i < uboComputeBuffers.size(); i++) {
    uboComputeBuffers[i] =
        std::make_unique<ODBuffer>(m_device, sizeof(ComputeShaderUbo), 1,
                                   VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
                                   VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
    uboComputeBuffers[i]->map();
  }

  std::vector<std::unique_ptr<ODBuffer>> uboBuffers(
      ODSwapChain::MAX_FRAMES_IN_FLIGHT);
  for (int i = 0; i < uboBuffers.size(); i++) {
    uboBuffers[i] = std::make_unique<ODBuffer>(
        m_device, sizeof(GlobalUbo), 1, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT);
    uboBuffers[i]->map();
  }

  auto globalSetLayout =
      ODDescriptorSetLayout::Builder(m_device)
  ...
  std::vector<VkDescriptorSet> globalDescriptorSets(
      ODSwapChain::MAX_FRAMES_IN_FLIGHT); // 1 descriptor set per frame
  for (int i = 0; i < globalDescriptorSets.size(); i++) {
    auto bufferInfo = uboBuffers[i]->descriptorInfo();
    auto imageInfo = m_textureHandler->descriptorInfo();
    auto computeBufferInfo0 =
        m_particleSystem.getParticleBuffers()[i]->descriptorInfo();
    auto computeBufferInfo1 =
        m_particleSystem.getParticleBuffers()[(i + 1) % 2]->descriptorInfo();
  ...
  PointLightSystem pointLightSystem(m_device,
                                    ...
                                    globalSetLayout->getDescriptorSetLayout());
  // GridSystem gridSystem(m_device, m_renderer.getSwapChainRenderPass(),
  // globalSetLayout->getDescriptorSetLayout());
  ...
  m_cameraObject.camera->setViewTarget(glm::vec3(-1.0f, -2.0f, 2.0f),
  ...
  m_cameraObject.camera->setPerspectiveProjection(glm::radians(50.0f), aspect,
                                                  0.1f, 1000.0f);
  ...
  while (!m_window.shouldClose()) {
    ...
    cameraController.HandleInputs(m_window.getGLFWWindow(), deltaTime,
                                  m_cameraObject);
    m_cameraObject.camera->setViewYXZ(m_cameraObject.transform.translation,
                                      m_cameraObject.transform.rotation);
    float aspect = m_renderer.getAspectRatio();
    ...
    if (commandBuffer !=
        nullptr) { // If swapChain needs to be recreated, returns a nullptr
      FrameInfo frameInfo{
          frameIndex,
          deltaTime,
          commandBuffer,
          *m_cameraObject.camera,
          ...
          m_gameObjects,
          m_particleSystem.getParticleBuffers()[(frameIndex + 1) % 2]
              ->getBuffer()};
      ...
      // compute
      ComputeShaderFrameInfo computeFrameInfo{deltaTime, commandBuffer,
                                              globalDescriptorSets[frameIndex]};
      ...
      // compute
      gpuParticleSystem.compute(frameInfo,
                                m_renderer.getCurrentComputeCommandBuffers(),
      ...
      if (renderFinishedSemaphore != VK_NULL_HANDLE) {
        ...
        vkWaitForFences(m_device.device(), 1,
                        &m_transitionFences[currentImageIndex], VK_TRUE,
                        UINT64_MAX);
        vkResetFences(m_device.device(), 1,
                      &m_transitionFences[currentImageIndex]);

        VkCommandBuffer transitionCmd =
            m_transitionCommandBuffers[currentImageIndex];
        ...
        VkImage swapChainImage =
            m_renderer.getSwapChain().getImages()[currentImageIndex];
        ...
        VkSemaphore transitionSemaphore =
            m_transitionSemaphores[currentImageIndex];
        submitInfo.signalSemaphoreCount = 1;
        submitInfo.pSignalSemaphores = &transitionSemaphore;

        vkQueueSubmit(m_device.graphicsQueue(), 1, &submitInfo,
                      m_transitionFences[currentImageIndex]);
    ...
    // size_t bufferSize = sizeof(ODParticles::Particle) *
    // ODParticles::PARTICLE_COUNT; std::cout << "Contents of
    // m_computeBuffers[0]:" << std::endl; debugBuffer(m_device,
    // m_computeBuffers[0]->getBuffer(), bufferSize); std::cout << "Contents of
    // m_computeBuffers[1]:" << std::endl; debugBuffer(m_device,
    // m_computeBuffers[1]->getBuffer(), bufferSize);
  }
  vkDeviceWaitIdle(m_device.device());
}

std::shared_ptr<ODModel>
App::createModelFromFile(const std::string &modelPath) {
...
void App::transitionImageLayout(VkCommandBuffer commandBuffer, VkImage image,
...
void App::debugBuffer(ODDevice &device, VkBuffer srcBuffer, size_t size) {
  ...
  // Lire et afficher
  ODParticles::Particle *data = reinterpret_cast<ODParticles::Particle *>(
      stagingBuffer.getMappedMemory());
  for (size_t i = 0;
       i < std::min(size_t(10), size_t(ODParticles::PARTICLE_COUNT));
       ++i) { // Affiche les 10 premières
    std::cout << "Particle " << i << ": Pos(" << data[i].position.x << ", "
              << data[i].position.y << "), Vel(" << data[i].velocity.x << ", "
              << data[i].velocity.y << "), Color(" << data[i].color.r << ", "
              << data[i].color.g << ", " << data[i].color.b << ")" << std::endl;
  }
