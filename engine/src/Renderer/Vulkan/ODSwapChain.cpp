...
ODSwapChain::ODSwapChain(ODDevice &deviceRef, VkExtent2D windowExtent,
...
VkResult ODSwapChain::acquireNextImage(uint32_t *imageIndex,
                                       uint32_t frameIndex) {
  vkWaitForFences(device.device(), 1, &inFlightFences[frameIndex], VK_TRUE,
...
VkResult ODSwapChain::submitCommandBuffers(const VkCommandBuffer *buffers,
                                           uint32_t *imageIndex,
                                           uint32_t frameIndex) {
  ...
  // IMPORTANT: Utilise currentFrame pour wait (correspond à acquire) mais
  // imageIndex pour signal
  VkSemaphore waitSemaphores[] = {computeFinishedSemaphores_[frameIndex],
...
// VkResult ODSwapChain::submitComputeCommandBuffers(const VkCommandBuffer
// *buffers, uint32_t *imageIndex)
// {
//     vkWaitForFences(device.device(), 1,
//     &computeInFlightFences_[currentFrame], VK_TRUE, UINT64_MAX);

//         vkResetFences(device.device(), 1,
//         &computeInFlightFences_[currentFrame]);
...
//         &computeFinishedSemaphores_[currentFrame];

//         if (vkQueueSubmit(computeQueue, 1, &submitInfo,
//         computeInFlightFences_[currentFrame]) != VK_SUCCESS) {
//             throw std::runtime_error("failed to submit compute command
//             buffer!");
//         };
// }

// void ODSwapChain::recordComputeCommandBuffer(VkCommandBuffer commandBuffer,
// FrameInfo &frameInfo) {
//   VkCommandBufferBeginInfo beginInfo{};
//   beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;

//   if (vkBeginCommandBuffer(commandBuffer, &beginInfo) != VK_SUCCESS) {
//       throw std::runtime_error("failed to begin recording compute command
//       buffer!");
//   }
...
void ODSwapChain::waitForImageToBeAvailable(uint32_t imageIndex) {
  ...
  if (imagesInFlight[imageIndex] != VK_NULL_HANDLE) {
    ...
    VkResult result =
        vkGetFenceStatus(device.device(), imagesInFlight[imageIndex]);
...
void ODSwapChain::createSwapChain() {
  ...
  createInfo.imageUsage =
      ...
      VK_IMAGE_USAGE_SAMPLED_BIT; // !! vérifier avec ancienne valeur
...
VkImageView ODSwapChain::createImageView(ODDevice &app_device, VkImage image,
                                         VkFormat format,
  ...
  if (vkCreateImageView(app_device.device(), &viewInfo, nullptr, &imageView) !=
      VK_SUCCESS) {
...
void ODSwapChain::createRenderPass() {
  ...
  colorAttachmentResolve.finalLayout =
      VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL; // modifié
...
void ODSwapChain::createFramebuffers() {
  ...
  for (size_t i = 0; i < imageCount(); i++) {
    std::array<VkImageView, 3> attachments = {
        msaaColorImageView, depthImageViews[i], swapChainImageViews[i]};
...
void ODSwapChain::createDepthResources() {
  ...
  for (int i = 0; i < depthImages.size(); i++) {
    createImage(device, swapChainExtent.width, swapChainExtent.height, 1,
                device.getMsaaSamples(), depthFormat, VK_IMAGE_TILING_OPTIMAL,
                VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
                VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, depthImages[i],
                depthImageMemorys[i]);
    depthImageViews[i] = createImageView(
        device, depthImages[i], depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT,
        1); // Ajout de l'assignation depthImageViews[i] = ...
  }
}

void ODSwapChain::createImage(ODDevice &app_device, uint32_t width,
  ...
  if (vkCreateImage(app_device.device(), &imageInfo, nullptr, &image) !=
      VK_SUCCESS) {
  ...
  allocInfo.memoryTypeIndex = app_device.findMemoryType(
      memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);

  if (vkAllocateMemory(app_device.device(), &allocInfo, nullptr,
...
void ODSwapChain::createMsaaColorResources() {
  ...
  createImage(device, swapChainExtent.width, swapChainExtent.height, 1,
              device.getMsaaSamples(), colorFormat, VK_IMAGE_TILING_OPTIMAL,
              VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT |
                  VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT,
              VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, msaaColorImage,
              msaaColorImageMemory);
  msaaColorImageView =
      createImageView(device, msaaColorImage, colorFormat,
                      VK_IMAGE_ASPECT_COLOR_BIT, 1); // vérifier
}
...
void ODSwapChain::createSyncObjects() {
  ...
  for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
    if (vkCreateFence(device.device(), &fenceInfo, nullptr,
                      &inFlightFences[i]) != VK_SUCCESS) {
      throw std::runtime_error("failed to create inFlight fence!");
    }
    if (vkCreateSemaphore(device.device(), &semaphoreInfo, nullptr,
                          &computeFinishedSemaphores_[i]) != VK_SUCCESS ||
        vkCreateFence(device.device(), &fenceInfo, nullptr,
                      &computeInFlightFences_[i]) != VK_SUCCESS) {
...
VkSurfaceFormatKHR ODSwapChain::chooseSwapSurfaceFormat(
  ...
  for (const auto &availableFormat : availableFormats) {
    if (availableFormat.format ==
            VK_FORMAT_B8G8R8A8_SRGB && // ou VK_FORMAT_B8G8R8A8_UNORM pour pas
                                       // de correction GAMMA
...
VkPresentModeKHR ODSwapChain::chooseSwapPresentMode(
...
VkExtent2D
ODSwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
...
VkResult ODSwapChain::submitCommandBuffersWithoutPresent(
...
void ODSwapChain::presentFrameWithSemaphore(uint32_t *imageIndex,
