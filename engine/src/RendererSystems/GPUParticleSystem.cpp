...
GPUParticleSystem::GPUParticleSystem(ODDevice &device, VkRenderPass renderPass,
...
void GPUParticleSystem::createComputePipelineLayout(
    VkDescriptorSetLayout globalSetLayout) {
...
void GPUParticleSystem::createComputePipeline(VkRenderPass renderPass) {
  assert(m_computePipelineLayout != nullptr &&
         "Pipeline layout must be created before creating the pipeline");
  ...
  m_odComputePipeline = std::make_unique<ODComputePipeline>(
      m_device, ENGINE_PATH "/shaders/compiled/compute.spv", pipelineConfig);
}

void GPUParticleSystem::createGraphicsPipelineLayout(
    VkDescriptorSetLayout globalSetLayout) {
  std::vector<VkDescriptorSetLayout> descriptorSetLayouts{globalSetLayout};
...
void GPUParticleSystem::createGraphicsPipeline(VkRenderPass renderPass) {
  assert(m_graphicsPipelineLayout != nullptr &&
         "Pipeline layout must be created before creating the pipeline");
  ...
  pipelineConfig.attributeDescriptions =
      ODParticles::Particle::getAttributeDescriptions();
  pipelineConfig.bindingDescriptions =
      ODParticles::Particle::getBindingDescriptions();
  ...
  m_odGraphicsPipeline = std::make_unique<ODGraphicsPipeline>(
      m_device, ENGINE_PATH "/shaders/compiled/compute.vert.spv",
      ENGINE_PATH "/shaders/compiled/compute.frag.spv", pipelineConfig);
}
...
void GPUParticleSystem::render(FrameInfo &frameInfo) {
  ...
  vkCmdBindDescriptorSets(frameInfo.commandBuffer,
                          ...
                          m_graphicsPipelineLayout, 0, 1,
                          &frameInfo.globalDescriptorSet, 0, nullptr);

  VkDeviceSize offsets[] = {0};
  vkCmdBindVertexBuffers(frameInfo.commandBuffer, 0, 1,
                         &frameInfo.particleBuffer, offsets);

  vkCmdDraw(frameInfo.commandBuffer, ODParticles::PARTICLE_COUNT, 1, 0, 0);
}

void GPUParticleSystem::compute(
    FrameInfo &frameInfo, VkCommandBuffer commandBuffer,
    std::vector<VkSemaphore> computeFinishedSemaphores,
    std::vector<VkFence> computeInFlightFences) {

  int currentFrame = frameInfo.frameIndex;

  vkWaitForFences(m_device.device(), 1, &computeInFlightFences[currentFrame],
                  VK_TRUE, UINT64_MAX);
  ...
  if (vkQueueSubmit(m_device.computeQueue(), 1, &submitInfo,
                    computeInFlightFences[currentFrame]) != VK_SUCCESS) {
    throw std::runtime_error("failed to submit compute command buffer!");
  };
}

void GPUParticleSystem::recordComputeCommandBuffer(
