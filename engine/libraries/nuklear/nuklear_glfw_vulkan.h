...
struct nk_glfw_device {
    ...
    uint32_t framebuffer_width_pixels; // modifié
    uint32_t framebuffer_height_pixels; // modifié
};
...
NK_INTERN void nk_glfw3_create_framebuffers(struct nk_glfw_device *dev,
    ...
    dev->framebuffer_width_pixels  = framebuffer_width; // modifié
    dev->framebuffer_height_pixels = framebuffer_height; // modifié
    ...
    framebuffer_create_info.width = dev->framebuffer_width_pixels;
    framebuffer_create_info.height = dev->framebuffer_height_pixels;
...
VkSemaphore nk_glfw3_render(VkQueue graphics_queue, uint32_t buffer_index,
    ...
    render_pass_begin_nfo.renderArea.extent.width = (uint32_t)dev->framebuffer_width_pixels;
    render_pass_begin_nfo.renderArea.extent.height = (uint32_t)dev->framebuffer_height_pixels;
    ...
    viewport.width = (float)dev->framebuffer_width_pixels;
    viewport.height = (float)dev->framebuffer_height_pixels;
                            ...
                            &dev->uniform_descriptor_set, 0, NULL);
    {
        ...
        nk_draw_foreach(cmd, &glfw.ctx, &dev->cmds) {
            ...
            const float scaleX = (float)dev->framebuffer_width_pixels  / (float)glfw.width; // modifié
            const float scaleY = (float)dev->framebuffer_height_pixels / (float)glfw.height; // modifié

            scissor.offset.x     = (int32_t)(NK_MAX(cmd->clip_rect.x, 0.f) * scaleX); // modifié
            scissor.offset.y     = (int32_t)(NK_MAX(cmd->clip_rect.y, 0.f) * scaleY); // modifié
            scissor.extent.width = (uint32_t)(cmd->clip_rect.w * scaleX); // modifié
            scissor.extent.height= (uint32_t)(cmd->clip_rect.h * scaleY); // modifié
            uint32_t maxW = dev->framebuffer_width_pixels; // modifié
            uint32_t maxH = dev->framebuffer_height_pixels; // modifié
            if ((uint32_t)scissor.offset.x + scissor.extent.width > maxW) { // modifié
                scissor.extent.width = (maxW > (uint32_t)scissor.offset.x) // modifié
                    ? (maxW - (uint32_t)scissor.offset.x) : 0u; // modifié
            } // modifié
            if ((uint32_t)scissor.offset.y + scissor.extent.height > maxH) { // modifié
                scissor.extent.height = (maxH > (uint32_t)scissor.offset.y) // modifié
                    ? (maxH - (uint32_t)scissor.offset.y) : 0u; // modifié
            } // modifié
            vkCmdSetScissor(command_buffer, 0, 1, &scissor); // modifié
